...
export function AlimentationPage() {
  const stats = [
    ...
    { value: 23, label: 'Sautent régulièrement des repas', suffix: '%', color: 'red' as const },
    { value: '1/4', label: 'Ne mange pas à sa faim tous les jours', color: 'red' as const },
    ...
    { value: 1, label: 'Prix d\'un repas CROUS pour les boursiers', suffix: '€', color: 'green' as const },
  ];
  ...
  return (
    <div className="min-h-screen pt-28 pb-16">
      <div className="max-w-5xl mx-auto px-6">
        <Breadcrumb items={[{ label: 'Accueil', href: '/' }, { label: 'Thématiques', href: '/#themes' }, { label: 'Alimentation' }]} />
        ...
        >
          ...
          <div className="bg-[#f85149]/10 border border-[#f85149] rounded-xl p-4 -mt-8">
            <p className="text-white text-sm">
              <strong className="text-[#f85149]">48% des étudiants rencontrent des difficultés alimentaires.</strong> 1 étudiant sur 5 saute des repas régulièrement ou vit une précarité alimentaire sévère.
            </p>
          </div>
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <h2 className="text-3xl font-bold text-white mb-8">Témoignage : le poids de la honte</h2>
          <TestimonialCardLong
            quote="J'avais 40€ pour les 15 derniers jours du mois. 40€ pour manger, prendre le bus, et vivre. J'ai mangé des pâtes nature et du pain pendant 10 jours. Je pesais 54kg pour 1m72. Je n'en parlais à personne - ni à mes parents (pour ne pas les inquiéter), ni à mes amis (pour ne pas avoir l'air dans la misère). Une camarade m'a dit qu'il y avait une épicerie solidaire à 500 mètres de l'IUT. J'ai mis 3 semaines à oser y aller. La honte. Quand je suis entrée, il y avait une fille de ma promo. On s'est regardées sans rien dire. On est reparties chacune avec nos courses. On s'est parlé après. Elle vivait la même chose depuis des mois. On se privait toutes les deux alors que la solution était là, gratuite, à 5 minutes."
            name="Chloé"
            age={21}
            field="BUT Chimie"
            city="Toulouse"
        ...
        {/* Navigation */}
        <PageNavigation
          previousPage={{ title: 'Santé Mentale', href: '/sante-mentale' }}
          nextPage={{ title: 'Réussite', href: '/reussite' }}
        />
      </div>
    </div>
  );
